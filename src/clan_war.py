...
@on_command('turn_on_clan_war', aliases=('开启工会战', '开启公会战'), only_to_me=False)
async def turn_on_clan_war(session: CommandSession):

    current_qq = session.ctx['user_id']  # 获取发送当前指令的用户的qq

    # 判断公会战功能是否已经启动
    if MyBot.data.clan_war:
        await session.send(f'工会战功能已经开启，无需再次启动。')
    else:
        # 判断当前用户是否为设置的管理员
        if MyBot.data.check_on_if_op(current_qq):
            MyBot.data.change_clan_war()
            await session.send(f'公会战功能成功开启！')
        else:
            await session.send(f'SORRY！非管理员成员不能开启。')
...
@on_command('turn_off_clan_war', aliases=('关闭工会战', '关闭公会战'), only_to_me=False)
async def turn_off_clan_war(session: CommandSession):
    ...
    if MyBot.data.clan_war:
        # 判断当前用户是否为设置的管理员
        if MyBot.data.check_on_if_op(current_qq):
            MyBot.data.reset_bot()
            await session.send(f'公会战功能成功关闭！')
        else:
            await session.send(f'SORRY！非管理员成员不能关闭。')
    else:

        await session.send(f'工会战功能已关闭，无需再次关闭。')
